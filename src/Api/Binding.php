...
class Binding extends AbstractApi
{
    ...
    public function all($vhost = null)
    {
        if ($vhost) {
            return $this->client->send(sprintf(
                    '/api/bindings/%s', urlencode($vhost))
            );
        }
    ...
    {
        return $this->client->send(
            sprintf('/api/bindings/%s/e/%s/q/%s', urlencode($vhost), urlencode($exchange), urlencode($queue))
        );
    }
    ...
    {
        return $this->client->send(
            sprintf('/api/bindings/%s/e/%s/e/%s', urlencode($vhost), urlencode($source), urlencode($destination))
        );
    }
    ...
    {
        return $this->client->send(
            sprintf('/api/bindings/%s/e/%s/q/%s/%s', urlencode($vhost), urlencode($exchange), urlencode($queue), urlencode($props))
        );
    }
    ...
    {
        return $this->client->send(
            sprintf('/api/bindings/%s/e/%s/e/%s/%s', urlencode($vhost), urlencode($source), urlencode($destination), urlencode($props))
        );
    }
